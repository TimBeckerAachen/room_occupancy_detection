...
def plot_misclassifications(df_index, y_true, y_pred, features_df=None, feature_to_plot=None):
    """
    ...
    results_df['show'] = results_df['error'] * results_df['true']  # Show only false negatives
    results_df['show2'] = results_df['error'] * (1 - results_df['true'])  # Show only false positives
    ...
    # daily_groups = list(results_df.groupby(pd.Grouper(freq='D')))
    ...
    for date, group in daily_groups:
        if group['error'].sum() > 0:
            weekday = group.index[0].strftime('%A') # date.strftime('%A')
...
def plot_daily_results(df, features_to_plot=None):
    """
    ...
    features_to_plot : str or list, default None
        Additional feature(s) to plot on secondary axis
    """
    if isinstance(features_to_plot, str) and features_to_plot in df.columns:
        features_to_plot = [features_to_plot]
    elif features_to_plot is None:
...
def plot_param_importances(cv_results):
